...
{
TODO: Remove this if it works by just JetEncodeValue

//Converts field value to a string representation suitable for use in SQL, depending on its type
function FieldValToStr(f: Field20): string;
var tp: TOleEnum;
begin
  tp := f.Type_;
  case tp of
    adEmpty:
      Result := 'NULL'; //what else
    adTinyInt, adSmallInt, adInteger, adBigInt,
    adUnsignedTinyInt, adUnsignedSmallInt, adUnsignedInt, adUnsignedBigInt:
      Result := IntToStr(f.Value);
    adSingle, adDouble:
      Result := FloatToStr(f.Value);
    adCurrency:
      Result := CurrToStr(f.Value);
    adDecimal, adNumeric:
      Result := FloatToStr(f.Value); //I suppose?
    adBoolean:
      Result := BoolToStr(f.Value, True).ToUpper;
    adBSTR,
    adChar, adVarChar, adLongVarChar,
    adWChar, adVarWChar, adLongVarWChar:
      Result := '"' + EscapeSqlStr(f.Value) + '"';
  else
   //The rest I have no friggin idea
    adError
    adGUID
    adDate
    adDBDate
    ...
    adVarNumeric
    adArray
    raise Exception.Create('Unsupported field type: '+IntToStr(tp));
  end;
end;
}
